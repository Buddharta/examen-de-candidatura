...
%%\end{teorema}
%%\begin{frame} Como observaci\'on, se puede apreciar del resultado anterior que en una variedad de K\"ahler conexa, la existencia
...
%%\begin{frame}  Inspirados por la demostraci\'on del teorema \ref{DPPT} del art\'iculo reciente \cite{DPPT}, creemos que existen
...
%%\begin{frame} Primero hay que recordar que una variedad compleja $M$ es de Poisson si existe una operaci\'on bilineal en el anillo de g\'ermenes de
...
%%\end{enumerate}
%%\begin{frame} Se observa que un germen de una funci\'on fija $H\in\mathcal{O}_M$ define un campo vectorial definido por $\xi_H(\cdot)=\{H,\cdot\}$.
...
%%\]
%%\begin{frame} Con esto, el rango de $\pi$ en un punto $p\in M$ se define como el entero positivo $r\in \zah^{+}$ que define la dimensi\'on del
...
%%        \]
%%\end{teorema}
%%\begin{frame} Observamos del teorema anterior que $f^{jk}(x)=0$. Este teorema define claramente una foliaci\'on natural en $(M,\pi)$ de hojas
...
%%\end{conjetura}
%%\begin{frame} No sabemos si es cierta o falsa o si es posible demostrar con lo que vamos a proponer, pero creemos que como una
